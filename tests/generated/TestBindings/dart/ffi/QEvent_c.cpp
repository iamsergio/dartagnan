...
static TestBindings_wrappersNS::QEvent_wrapper *fromWrapperPtr(void *ptr)
{
    return reinterpret_cast<TestBindings_wrappersNS::QEvent_wrapper *>(ptr);
}
extern "C" {
void c_QEvent_Finalizer(void *cppObj)
{
    delete reinterpret_cast<TestBindings_wrappersNS::QEvent_wrapper *>(cppObj);
} // accept()
...
void *c_QEvent__clone(void *thisObj)
{
    const auto &result = [&] {
        ...
        auto wrapperPtr = dynamic_cast<TestBindings_wrappersNS::QEvent_wrapper *>(targetPtr);
        if (wrapperPtr) {
...
{
    const auto &result = TestBindings_wrappersNS::QEvent_wrapper::registerEventType(hint);
    return result;
}
...
void c_QEvent__registerVirtualMethodCallback(void *ptr, void *callback, int methodId)
{
    ...
    switch (methodId) {
    case 643:
        wrapper->m_cloneCallback =
            reinterpret_cast<TestBindings_wrappersNS::QEvent_wrapper::Callback_clone>(callback);
        break;
    case 651:
        wrapper->m_setAcceptedCallback =
            reinterpret_cast<TestBindings_wrappersNS::QEvent_wrapper::Callback_setAccepted>(
                callback);
        break;
    }
}
